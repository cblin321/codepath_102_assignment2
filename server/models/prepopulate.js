...
async function initDb() {
    await clearDb()
    await pool.query(`CREATE TABLE IF NOT EXISTS editors (
    ...
    data.forEach(async editor => {
        console.log(editor.name, editor.description, editor.image, editor.link)
        await editorModel.addOne(editor.name, editor.description, editor.image, editor.link)
    })

}

async function clearDb() {
    await pool.query(`DROP TABLE IF EXISTS editors;`)
